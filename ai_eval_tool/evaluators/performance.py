...
class PerformanceEvaluator(EvaluatorBase):
    ...
    def evaluate(self, data_df: pl.DataFrame) -> EvaluationResult:
        ...
        if perf_df.is_empty():
            ...
            return EvaluationResult(metrics={"warning": "No data for performance eval"}, plots={}, extra_data={})

        # Overall Performance Statistics (on total_time_ms)
        total_time_series = perf_df["total_time_ms"]
        if total_time_series.is_empty() or total_time_series.null_count() == total_time_series.len():
        ...
        if metrics["perf_mean_total_time_ms"] is not None and \
           metrics["perf_std_total_time_ms"] is not None and \
           metrics["perf_mean_total_time_ms"] > 0:
            metrics["perf_cv_total_time_ms"] = metrics["perf_std_total_time_ms"] / metrics["perf_mean_total_time_ms"]
        else:
        ...
        if metrics["perf_median_total_time_ms"] is not None and \
           metrics["perf_max_total_time_ms"] is not None and \
           metrics["perf_median_total_time_ms"] > 0 :
            metrics["perf_worst_case_amplification"] = metrics["perf_max_total_time_ms"] / metrics["perf_median_total_time_ms"]
        else:
            metrics["perf_worst_case_amplification"] = None


        # High latency occurrence (e.g., > mean + 3*std)
        if metrics["perf_mean_total_time_ms"] is not None and metrics["perf_std_total_time_ms"] is not None:
            threshold = metrics["perf_mean_total_time_ms"] + 3 * metrics["perf_std_total_time_ms"]
            high_latency_count = total_time_series.filter(total_time_series > threshold).len()
            metrics["perf_high_latency_rate"] = high_latency_count / len(total_time_series) if len(total_time_series) > 0 else 0

            # Store top N high latency samples (image_id, loop, total_time_ms)
            # This is an example of 'extra_data'
            if high_latency_count > 0:
                top_slow_samples = perf_df.filter(pl.col("total_time_ms") > threshold) \
                                          .sort("total_time_ms", descending=True) \
                                          .head(10) \
                                          .select(["image_id", "loop", "total_time_ms", "pre_time_ms", "inference_time_ms", "post_time_ms"])
        ...
        for component in time_components:
            ...
            if comp_series.is_empty() or comp_series.null_count() == comp_series.len():
                ...
                cv_comp = (std_comp / mean_comp) if mean_comp and mean_comp > 0 and std_comp is not None else None
                percentage_comp = (mean_comp / total_mean_time * 100) if total_mean_time and total_mean_time > 0 and mean_comp is not None else None
...
"""
    ...
    data = {
        "loop": [1, 1, 1, 2, 2],
        "image_id": ["img1", "img1", "img2", "img1", "img2"], # img1 in loop 1 is duplicated
    ...
    results = perf_evaluator.evaluate(df.clone()) # Clone df as evaluate might modify or subset it
    ...
    expected_mean = np.mean([115, 138, 126, 150]) # (115+138+126+150)/4 = 529/4 = 132.25
    assert abs(results.metrics["perf_mean_total_time_ms"] - expected_mean) < 1e-6, \
    ...
    empty_df_for_test = pl.DataFrame({
        "loop": [], "image_id": [], "pre_time_ms": [], "inference_time_ms": [], "post_time_ms": [], "total_time_ms": []
    }, schema={
        "loop": pl.Int64, "image_id": pl.Utf8, "pre_time_ms": pl.Float64, "inference_time_ms": pl.Float64,
        "post_time_ms": pl.Float64, "total_time_ms": pl.Float64
    })
    ...
    null_df_data = {
        "loop": [1, 1], "image_id": ["img1", "img2"],
        "pre_time_ms": [10, 12], "inference_time_ms": [100, 120], "post_time_ms": [5, 6],
        "total_time_ms": [None, None],
    }
    null_df = pl.DataFrame(null_df_data).with_columns(pl.col("total_time_ms").cast(pl.Float64))
    results_null = perf_evaluator.evaluate(null_df)
    print("\nPerformance Metrics (Null total_time_ms):")
    for k, v in results_null.metrics.items():
        print(f"  {k}: {v}")
    assert "warning" in results_null.metrics or results_null.metrics.get("perf_mean_total_time_ms") is None
