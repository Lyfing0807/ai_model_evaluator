...
def rbox_to_shapely_polygon(rbox: List[float]) -> Polygon:
    ...
    origin_box = Polygon([(-half_w, -half_h), (-half_w, half_h), (half_w, half_h), (half_w, -half_h)])
...
def calculate_riou_shapely(rbox1_params: List[float], rbox2_params: List[float]) -> float:
    ...
    try:
        ...
        if union_area == 0:
            return 0.0 if intersection_area == 0 else 1.0 # Or handle as error if union is 0 but intersection isn't
        return intersection_area / union_area
    except Exception as e:
        logger.error(f"Error in calculate_riou_shapely: {e}. Rbox1: {rbox1_params}, Rbox2: {rbox2_params}", exc_info=True)
        return 0.0
...
class RotatedDetectionStabilityEvaluator(StabilityEvaluatorBase):
    def __init__(self, config: MainConfig):
        ...
        logger.info(f"RotatedDetectionStabilityEvaluator initialized with RIoU threshold: {self.riou_threshold} using Shapely.")
        ...
        for loop_idx in range(1, len(loop_ids)):
            ...
            if not active_tracks_list or not detections_in_current_loop:
                for det_data in detections_in_current_loop:
                    new_id = next_track_id; new_current_tracks_for_next_iter[new_id] = det_data
                    tracked_detections_list.append({**det_data, "tracked_object_id": new_id}); next_track_id += 1
                current_tracks = new_current_tracks_for_next_iter; continue
            ...
            for i, prev_det in enumerate(active_tracks_list):
                for j, curr_det in enumerate(detections_in_current_loop):
                    ...
                    if prev_det.get("internal_rbbox") and curr_det.get("internal_rbbox"):
                        riou = calculate_riou_shapely(prev_det["internal_rbbox"], curr_det["internal_rbbox"])
            ...
            for r, c in zip(row_ind, col_ind):
                if (1.0 - cost_matrix[r,c]) >= self.riou_threshold:
                    prev_det_matched = active_tracks_list[r]; curr_det_matched = detections_in_current_loop[c]
                    original_track_id = -1
                    for tid, track_data_val in current_tracks.items():
                        if track_data_val["_original_idx_temp"] == prev_det_matched["_original_idx_temp"]: original_track_id = tid; break
            ...
            for c_idx, curr_det_unmatched in enumerate(detections_in_current_loop):
                if c_idx not in matched_current_indices:
                    ...
                    new_current_tracks_for_next_iter[new_id] = curr_det_unmatched; next_track_id += 1
    ...
    def evaluate(self, data_df: pl.DataFrame) -> EvaluationResult:
        logger.info("Starting rotated detection stability evaluation (using Shapely RIoU)...")
        ...
        if not all(col in data_df.columns for col in required_cols):
            ...
            return EvaluationResult(metrics={"error": f"Missing required columns for RDet: {missing}"})
        ...
        for (img_id, trk_id), grp in tracked_df.group_by(["image_id", "tracked_object_id"], maintain_order=False):
            ...
            img_total_loops_series = total_loops_img.filter(pl.col("image_id") == img_id)["total_loops"]
            if img_total_loops_series.is_empty(): continue # Should not happen if img_id exists
            ...
            centers_x = grp.select(pl.col("internal_rbbox").list.get(0).alias("cx"))["cx"].drop_nulls()
            centers_y = grp.select(pl.col("internal_rbbox").list.get(1).alias("cy"))["cy"].drop_nulls()
            widths = grp.select(pl.col("internal_rbbox").list.get(2).alias("w"))["w"].drop_nulls()
            heights = grp.select(pl.col("internal_rbbox").list.get(3).alias("h"))["h"].drop_nulls()

            mean_cx, mean_cy = centers_x.mean() if centers_x.len() > 0 else None, centers_y.mean() if centers_y.len() > 0 else None
            center_drifts_px = None
            if mean_cx is not None and mean_cy is not None and centers_x.len() > 0 :
                 center_drifts = ((centers_x - mean_cx)**2 + (centers_y - mean_cy)**2).sqrt()
        ...
        if not object_metrics_list: return EvaluationResult(metrics={"warning":"No RDet object metrics calculated."})
        stats_df = pl.DataFrame(object_metrics_list)

        final_metrics = {
            f"rdt_stab_mean_{col.replace('_placeholder','')}": stats_df[col].mean() for col in
            ["appearance_consistency", "mean_riou_consistency", "angle_std_dev",
    ...
    def _calculate_stability_score(self, metrics: Dict[str, Any], details_df: pl.DataFrame):
        ...
        if not self.eval_params or not self.eval_params.scoring_weights:
            logger.warning("Rotated detection stability scoring weights not configured. Skipping score calculation.")
            return
        ...
        if score_riou_consistency is None: score_riou_consistency = 0.0
        ...
        if score_angle_stability is None: score_angle_stability = 0.0
        ...
        current_weights_sum = sum(weights.get(k,0.0) for k in ["riou_consistency", "angle_stability", "existence_stability"])
        if current_weights_sum > 1e-6 and abs(current_weights_sum - 1.0) > 1e-6:
            ...
            logger.warning(f"Rotated detection stability weights used ({ {k:weights.get(k) for k in ['riou_consistency', 'angle_stability', 'existence_stability']} }) do not sum to 1. Normalizing score based on used weights.")
            s_stability_rdet = s_stability_rdet / current_weights_sum if current_weights_sum > 1e-9 else 0.0
        ...
        logger.info(f"Rotated Detection Stability Scores: RIoUCons={score_riou_consistency:.2f}, AngleStab={score_angle_stability:.2f}, Existence={score_existence:.2f}, Overall={s_stability_rdet:.2f}")
...
"""
    ...
    test_df = pl.DataFrame({
        "id": ["img1"]*2 + ["img2"]*2, "lp": [1,2,1,2],
        "internal_rbbox": [ # cx,cy,w,h,angle
            [50,50,20,10,0], [50,50,20,10,5], # img1, objA, slightly rotated
            [100,100,30,15,45], [100,100,30,15,45] # img2, objB, identical
        ],
        "cat": [0,0,1,1], "scr": [0.9,0.88,0.95,0.96]
    })
    ...
    assert results.metrics["rdt_stab_score_overall"] >= 0 and results.metrics["rdt_stab_score_overall"] <= 100

    if dummy_cfg_path.exists(): dummy_cfg_path.unlink()
    import shutil
    if Path("./test_rdet_stab_shapely_output").exists(): shutil.rmtree("./test_rdet_stab_shapely_output")
```
