...
from .detection import (
    calculate_iou,  # Can reuse for bbox matching if needed outside motmetrics
)
...
class TrackingStabilityEvaluator(StabilityEvaluatorBase):
    def __init__(self, config: MainConfig):
        ...
        if not isinstance(self.eval_params, TrackingEvaluationParams):
            raise ValueError(
                "Tracking evaluation parameters missing or incorrect type."
            )
    ...
    def evaluate(self, data_df: pl.DataFrame) -> EvaluationResult:
        ...
        has_gt = (
            "object_id_gt" in data_df.columns and "internal_bbox_gt" in data_df.columns
        )
        ...
        if not all(col in data_df.columns for col in required_pred_cols):
            ...
            return EvaluationResult(
                metrics={
                    "error": f"Missing required prediction columns for Tracking: {missing}"
                }
            )

        if data_df.is_empty():
            return EvaluationResult(
                metrics={"warning": "Empty input data for Tracking evaluation."}
            )
        ...
        if has_gt:
            logger.info(
                "Ground truth columns found. Calculating MOT metrics (MOTA, MOTP, IDF1, etc.)."
            )
            ...
            loops = (
                data_df["loop"].unique().to_list() if "loop" in data_df.columns else [1]
            )  # Default to one loop
            loop_mot_metrics_summary = []

            for loop_val in loops:
                loop_df = (
                    data_df.filter(pl.col("loop") == loop_val)
                    if "loop" in data_df.columns
                    else data_df
                )
                if loop_df.is_empty():
                    continue
                ...
                for frame_id_val, frame_df in loop_df.group_by(
                    "frame_id", maintain_order=True
                ):
                    gt_ids = frame_df.get_column("object_id_gt").drop_nulls().to_list()
                    pred_ids = (
                        frame_df.get_column("object_id_pred").drop_nulls().to_list()
                    )

                    gt_bboxes = frame_df.filter(pl.col("object_id_gt").is_not_null())[
                        "internal_bbox_gt"
                    ].to_list()
                    pred_bboxes = frame_df.filter(
                        pl.col("object_id_pred").is_not_null()
                    )["internal_bbox_pred"].to_list()

                    if not pred_ids:  # No predictions in this frame
                        acc.update(
                            gt_ids, [], [], frameid=frame_id_val
                        )  # All GTs are misses
                        continue
                    if not gt_ids:  # No GTs in this frame
                        acc.update(
                            [], pred_ids, [[] for _ in pred_ids], frameid=frame_id_val
                        )  # All preds are FPs
                        continue
                    ...
                    acc.update(gt_ids, pred_ids, distances, frameid=frame_id_val)

                mh = mm.metrics.create()
                summary = mh.compute(
                    acc,
                    metrics=mm.metrics.motchallenge_metrics,
                    name=f"mot_summary_loop_{loop_val}",
                )
            ...
            if loop_mot_metrics_summary:
                ...
                avg_mota = np.nanmean(
                    [
                        s["mota"].iloc[0]
                        for s in loop_mot_metrics_summary
                        if "mota" in s.columns and not s["mota"].empty
                    ]
                )
                avg_motp = np.nanmean(
                    [
                        s["motp"].iloc[0]
                        for s in loop_mot_metrics_summary
                        if "motp" in s.columns and not s["motp"].empty
                    ]
                )
                avg_idf1 = np.nanmean(
                    [
                        s["idf1"].iloc[0]
                        for s in loop_mot_metrics_summary
                        if "idf1" in s.columns and not s["idf1"].empty
                    ]
                )
                # ... add others like num_switches, num_false_positives, num_misses etc.

                metrics["trk_mota"] = (
                    float(avg_mota) if not np.isnan(avg_mota) else None
                )
                metrics["trk_motp"] = (
                    float(avg_motp) if not np.isnan(avg_motp) else None
                )
                metrics["trk_idf1"] = (
                    float(avg_idf1) if not np.isnan(avg_idf1) else None
                )
                ...
                extra_data["motmetrics_summary_per_loop"] = (
                    loop_mot_metrics_summary  # Store detailed pandas summaries
                )
                logger.info(
                    f"MOT Metrics (avg across {len(loops)} loops): MOTA={metrics.get('trk_mota')}, MOTP={metrics.get('trk_motp')}, IDF1={metrics.get('trk_idf1')}"
                )
            else:
                logger.warning(
                    "No MOT metrics could be computed (e.g. no valid loops or frames)."
                )

        else:  # No Ground Truth
            logger.warning(
                "No ground truth track IDs (object_id_gt) or bboxes (internal_bbox_gt) found. Skipping MOT metrics."
            )
        ...
        self._calculate_stability_score(
            metrics, data_df
        )  # Pass data_df for now, may need details_df later
    ...
    def _calculate_stability_score(
        ...
        score_mota_stability = normalize_metric_to_score(
            metrics.get(
                "trk_mota"
            ),  # MOTA is typically 0-1 (or 0-100 if already scaled)
            is_0_1_rate_lower_better=False,  # Higher MOTA is better
        )
        if score_mota_stability is None:
            score_mota_stability = 0.0
        ...
        logger.info(
            f"Tracking Stability Score (Overall): {metrics['trk_stab_score_overall']:.2f}"
        )
...
if __name__ == "__main__":
    ...
    from ai_eval_tool.utils.scoring_utils import (
        normalize_metric_to_score,  # For direct import if needed
    )
...
"""
    ...
    test_df = pl.DataFrame(
        test_data
    ).drop_nulls()  # Drop rows where all GT/Pred specific are null if any
    ...
    if Path("./test_track_stab_output").exists():
        shutil.rmtree("./test_track_stab_output")
