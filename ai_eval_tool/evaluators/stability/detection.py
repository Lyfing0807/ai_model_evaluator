...
class DetectionStabilityEvaluator(StabilityEvaluatorBase):
    def __init__(self, config: MainConfig):
        ...
        if not self.eval_params or not hasattr(self.eval_params, "iou_threshold"):
            logger.error(
                "Detection evaluation parameters (e.g., iou_threshold) not correctly configured."
            )
            raise ValueError(
                "Detection evaluation parameters missing or of incorrect type."
            )
        self.iou_threshold: float = self.eval_params.iou_threshold  # type: ignore
        logger.info(
            f"DetectionStabilityEvaluator initialized with IoU threshold: {self.iou_threshold}."
        )
    ...
    def _track_objects_for_image(self, image_df: pl.DataFrame) -> pl.DataFrame:
        ...
        if (
            ...
            or "internal_bbox" not in image_df.columns
        ):
            logger.warning(
                f"Skipping tracking for image due to missing columns or empty data. Image ID: {image_df['image_id'][0] if not image_df.is_empty() else 'Unknown'}"
            )
        ...
        detections_by_loop: Dict[Any, List[Dict[str, Any]]] = defaultdict(
            list
        )  # Key is loop_id
        ...
        for loop_idx in range(1, len(loop_ids)):
            ...
            if not active_tracks_from_prev_loop_list or not detections_in_current_loop:
                for det_data in detections_in_current_loop:  # All are new tracks
                    ...
                    tracked_detections_list.append(
                        {**det_data, "tracked_object_id": new_track_id_for_det}
                    )
                    next_track_id += 1
            ...
            for i, prev_det_data in enumerate(active_tracks_from_prev_loop_list):
                for j, curr_det_data in enumerate(detections_in_current_loop):
                    iou = calculate_iou(
                        prev_det_data["internal_bbox"], curr_det_data["internal_bbox"]
                    )
                    if iou > 0:
            ...
            for r, c in zip(row_ind, col_ind):
                ...
                if iou >= self.iou_threshold:
                    ...
                    ):  # current_tracks holds data from previous loop
                        if (
                            track_data_val["_original_idx_temp"]
                            == prev_det_data_matched["_original_idx_temp"]
                        ):
                            original_track_id = tid
                            break
                    if (
                        original_track_id == -1
                    ):  # Should not happen if current_tracks is built correctly
                        logger.error(
                            "Could not find original track ID for a matched previous detection."
                        )
                        continue

                    tracked_detections_list.append(
                        {
                            **curr_det_data_matched,
                            "tracked_object_id": original_track_id,
                        }
                    )
            ...
            for c_idx, curr_det_data_unmatched in enumerate(detections_in_current_loop):
                if c_idx not in matched_current_det_indices:  # New track
                    ...
                    tracked_detections_list.append(
                        {**curr_det_data_unmatched, "tracked_object_id": new_id}
                    )
    ...
    def _calculate_average_box(self, group_df: pl.DataFrame) -> Optional[List[float]]:
        ...
        if not bboxes_list:
            return None  # Handle if all bboxes were null for this group
    ...
    def _track_objects_batch_optimized(self, data_df: pl.DataFrame) -> pl.DataFrame:
        ...
        for i in range(0, len(unique_image_ids), batch_size):
            ...
            if i % (batch_size * 10) == 0:  # Log progress every 1000 images
                logger.info(
                    f"Processed {min(i + batch_size, len(unique_image_ids))}/{len(unique_image_ids)} images"
                )
    ...
    def _track_objects_for_image_optimized(
        self, image_df: pl.DataFrame
    ) -> pl.DataFrame:
        """
        ...
        return image_df.with_columns(
            pl.Series("tracked_object_id", tracked_ids, dtype=pl.Int64)
        )

    def _match_detections_optimized(
        self,
    ...
    def _compute_iou_matrix_vectorized(
        self, bboxes1: List[List[float]], bboxes2: List[List[float]]
    ) -> np.ndarray:
        """
    ...
    def evaluate(self, data_df: pl.DataFrame) -> EvaluationResult:
        logger.info(
            f"Starting detection stability evaluation for model type {self.model_type}..."
        )
        ...
        if data_df.is_empty():
            logger.warning(
                "Input DataFrame is empty for detection stability evaluation."
            )
            return EvaluationResult(
                metrics={"warning": "Empty input data for detection stability."}
            )
        ...
        if missing_cols:
            logger.error(
                f"Detection stability evaluation requires columns: {required_cols}. Missing: {missing_cols}"
            )
            return EvaluationResult(
                metrics={
                    "error": f"Missing columns for detection stability: {missing_cols}"
                }
            )
        ...
        try:
            ...
            if bbox_sample is not None and (
                not isinstance(bbox_sample, list) or len(bbox_sample) != 4
            ):
                logger.error(
                    f"Invalid bbox format. Expected list of 4 numbers, got: {type(bbox_sample)}"
                )
                return EvaluationResult(
                    metrics={"error": "Invalid bbox format in internal_bbox column"}
                )
            ...
            if not score_stats.is_empty():
                ...
                if min_score < 0 or max_score > 1:
                    logger.warning(
                        f"Score values outside [0,1] range: min={min_score}, max={max_score}"
                    )
        ...
        if unique_loops < 2:
            logger.warning(
                f"Insufficient loops for stability analysis. Found {unique_loops}, need at least 2"
            )
            return EvaluationResult(
                metrics={"warning": f"Insufficient loops: {unique_loops} < 2"}
            )
        ...
        try:
            if unique_images > 1000:  # For large datasets, use batch processing
                logger.info(
                    f"Large dataset detected ({unique_images} images). Using batch processing for object tracking."
                )
            ...
            return EvaluationResult(
                metrics={"error": f"Object tracking failed: {str(e)}"}
            )
        ...
        if tracked_df.is_empty():
            ...
            return EvaluationResult(
                metrics={"warning": "No objects were successfully tracked."}
            )
        ...
        total_loops_per_image = data_df.group_by("image_id").agg(
            pl.col("loop").n_unique().alias("total_loops_for_image")
        )

        for (image_id, tracked_id), group in tracked_df.group_by(
            ...
            current_image_info = total_loops_per_image.filter(
                pl.col("image_id") == image_id
            )
            if current_image_info.is_empty():
                logger.warning(
                    f"Could not find total_loops_for_image for image_id {image_id}. Skipping object {tracked_id}."
                )
                continue
            total_img_loops = current_image_info["total_loops_for_image"][0]

            appearance_consistency = (
                num_loops_appeared / total_img_loops if total_img_loops > 0 else 0.0
            )
            ...
            if avg_box:
                avg_center = np.array(
                    [(avg_box[0] + avg_box[2]) / 2, (avg_box[1] + avg_box[3]) / 2]
                )
                avg_area = (avg_box[2] - avg_box[0]) * (avg_box[3] - avg_box[1])

                for det_box_list in (
                    group["internal_bbox"].drop_nulls().to_list()
                ):  # Ensure no null bboxes
                    det_box = np.array(det_box_list)
                    iou_consistencies.append(calculate_iou(det_box.tolist(), avg_box))

                    det_center = np.array(
                        [(det_box[0] + det_box[2]) / 2, (det_box[1] + det_box[3]) / 2]
                    )
            ...
            mean_iou_consistency = (
                np.mean(iou_consistencies) if iou_consistencies else None
            )
            mean_center_drift_px = np.mean(center_drifts) if center_drifts else None
            mean_size_jitter_ratio = (
                np.mean(size_jitters_area) if size_jitters_area else None
            )

            confidence_std = group[
                "score"
            ].std()  # Polars std handles nulls by ignoring them
            ...
            if num_loops_appeared > 1:
                ...
                for i in range(len(categories) - 1):
                    if (
                        ...
                        and categories[i] is not None
                        and categories[i + 1] is not None
                    ):  # Handle potential nulls
                        num_category_switches += 1
            category_switch_rate = (
                num_category_switches / (num_loops_appeared - 1)
                if num_loops_appeared > 1
                else 0.0
            )
        ...
        if not object_stability_metrics:
            ...
            return EvaluationResult(
                metrics={"warning": "No per-object stability metrics calculated."}
            )
        ...
        final_metrics = {
            ...
            if stability_stats_df[col].null_count()
            < len(stability_stats_df)  # Ensure there's some non-null data to mean
        }
        final_metrics["det_stab_num_total_tracked_instances"] = float(
            len(stability_stats_df)
        )
        final_metrics["det_stab_num_unique_objects_tracked_multiple_loops"] = float(
            stability_stats_df.filter(pl.col("num_loops_appeared") > 1)[
                "tracked_object_id"
            ].n_unique()
        )

        self._calculate_stability_score(final_metrics, stability_stats_df)
        ...
        return EvaluationResult(
            ...
            plots={},
            extra_data={
                "object_stability_details_df": stability_stats_df,
                "full_tracked_df_debug": tracked_df,
            },  # Keep full_tracked_df for now
        )
    ...
    def _calculate_stability_score(
        ...
        if not self.eval_params or not self.eval_params.scoring_weights:  # type: ignore
            logger.warning(
                "Detection stability scoring weights not configured. Skipping score calculation."
            )
            return
        ...
        score_iou_consistency = normalize_metric_to_score(
            metrics.get("det_stab_mean_iou_consistency"), is_0_1_rate_lower_better=False
        )
        if score_iou_consistency is None:
            score_iou_consistency = 0.0
        ...
        if total_weight > 1e-6 and abs(total_weight - 1.0) > 1e-6:
            logger.warning(
                f"Detection stability weights ({weights}) do not sum to 1. Normalizing score."
            )
        ...
        metrics["det_stab_score_position"] = (
            score_position  # Primarily IoU based for now
        )
        metrics["det_stab_score_confidence"] = score_confidence
        metrics["det_stab_score_category"] = score_category
        metrics["det_stab_score_overall"] = (
            s_stability_detection  # This is S_stability for detection
        )
        logger.info(
            f"Detection Stability Scores: Existence={score_existence:.2f}, Position={score_position:.2f}, Confidence={score_confidence:.2f}, Category={score_category:.2f}, Overall={s_stability_detection:.2f}"
        )
...
"""
    ...
    if "object_stability_details_df" in results.extra_data:
        ...
        assert (
            results.metrics.get("det_stab_num_unique_objects_tracked_multiple_loops")
            == 2.0
        )
    assert "det_stab_score_overall" in results.metrics  # Check if score was calculated
    assert (
        results.metrics["det_stab_score_overall"] >= 0
        and results.metrics["det_stab_score_overall"] <= 100
    )
