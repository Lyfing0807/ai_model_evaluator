...
def recall_at_k(y_true_list: List[Any], y_pred_list: List[Any], k: int) -> float:
    ...
    if not y_true_list: # No relevant items
        return 1.0 if not y_pred_list else 0.0 # Or 0.0 always if no true items? Define behavior.
    ...
    if not y_true_set: # Handles case where y_true_list might contain non-hashable or is effectively empty
...
class RankingStabilityEvaluator(StabilityEvaluatorBase):
    def __init__(self, config: MainConfig):
        ...
        self.k_values: List[int] = sorted(list(set(self.eval_params.k_values))) # Ensure unique and sorted
        logger.info(f"RankingStabilityEvaluator initialized with K values: {self.k_values}.")

    def evaluate(self, data_df: pl.DataFrame) -> EvaluationResult:
        logger.info("Starting ranking stability evaluation...")
        required_cols = ["query_id", "loop", "internal_item_id_pred_list", "internal_item_id_gt_list"]
        if not all(col in data_df.columns for col in required_cols):
            missing = [c for c in required_cols if c not in data_df.columns]
            return EvaluationResult(metrics={"error": f"Missing required columns for Ranking: {missing}"})

        if data_df.is_empty():
            return EvaluationResult(metrics={"warning": "Empty input data for Ranking eval."})
        ...
        for query_id_val, query_group_df in data_df.group_by("query_id", maintain_order=False):
            ...
            top_k_sets_across_loops: Dict[int, Dict[Any, Set[Any]]] = defaultdict(lambda: defaultdict(set))

            for loop_id_val, loop_group_df in query_group_df.group_by("loop", maintain_order=True): # Usually 1 row per query-loop
                if loop_group_df.is_empty(): continue
                row = loop_group_df.row(0, named=True) # Assuming one entry per query-loop pair
                ...
                if not pred_items and not gt_items: # Skip if both are empty for this query-loop
                    logger.debug(f"Skipping query {query_id_val}, loop {loop_id_val} due to empty pred and gt lists.")
                    continue
                ...
                for k in self.k_values:
                    ...
                    relevance_scores_for_pred = [1.0 if item in set(gt_items) else 0.0 for item in pred_items[:k]]
                    true_relevance_ideal = [1.0] * len(set(gt_items)) + [0.0] * (k - len(set(gt_items))) # Ideal ranking
                    ...
                    y_score_for_ndcg = np.array(pred_scores[:k] if pred_scores and len(pred_scores) >= k else np.arange(k, 0, -1), dtype=float)

                    current_ndcg = 0.0
                    if len(relevance_scores_for_pred) > 0 and len(y_score_for_ndcg) > 0 and len(relevance_scores_for_pred) == len(y_score_for_ndcg):
                         # Ensure y_true and y_score are 2D for sklearn
                        current_ndcg = ndcg_score(np.asarray([relevance_scores_for_pred]), np.asarray([y_score_for_ndcg]), k=k)
                    elif not relevance_scores_for_pred and not gt_items: # No relevant items and no predictions, perfect score
                        current_ndcg = 1.0

                    current_recall = recall_at_k(gt_items, pred_items, k)

                    all_query_loop_metrics.append({
                        "query_id": query_id_val, "loop": loop_id_val, "k": k,
                        "ndcg": current_ndcg, "recall": current_recall
                    })
                    metrics_across_loops_for_query[f"ndcg_at_{k}"].append(current_ndcg)
                    metrics_across_loops_for_query[f"recall_at_{k}"].append(current_recall)
                    top_k_sets_across_loops[k][loop_id_val] = set(pred_items[:k])

            # Calculate stability for this query_id
            if num_loops >= 2:
                query_stab_entry: Dict[str, Any] = {"query_id": query_id_val, "num_loops": num_loops}
                for k in self.k_values:
                    ndcg_list = metrics_across_loops_for_query.get(f"ndcg_at_{k}", [])
                    recall_list = metrics_across_loops_for_query.get(f"recall_at_{k}", [])
                    query_stab_entry[f"ndcg_at_{k}_std"] = np.std(ndcg_list) if len(ndcg_list) > 1 else 0.0
                    query_stab_entry[f"recall_at_{k}_std"] = np.std(recall_list) if len(recall_list) > 1 else 0.0
                    ...
                    if len(loop_ids_for_k) >= 2:
                        ...
                        for i in range(1, len(loop_ids_for_k)):
                            other_loop_set = top_k_sets_across_loops[k][loop_ids_for_k[i]]
                            jaccard_scores_for_k.append(jaccard_similarity_at_k(list(first_loop_set), list(other_loop_set), k)) # k is effectively list length here
                    query_stab_entry[f"jaccard_at_{k}_mean_vs_first_loop"] = np.mean(jaccard_scores_for_k) if jaccard_scores_for_k else (1.0 if num_loops ==1 else None)
        ...
        if all_query_loop_metrics:
            ...
            for k in self.k_values:
                final_metrics[f"rnk_mean_ndcg_at_{k}"] = loop_metrics_df.filter(pl.col("k") == k)["ndcg"].mean()
                final_metrics[f"rnk_mean_recall_at_{k}"] = loop_metrics_df.filter(pl.col("k") == k)["recall"].mean()

        if query_stability_metrics:
            query_stability_df = pl.DataFrame(query_stability_metrics)
            for k in self.k_values:
                final_metrics[f"rnk_stab_mean_ndcg_at_{k}_std"] = query_stability_df[f"ndcg_at_{k}_std"].mean()
                final_metrics[f"rnk_stab_mean_recall_at_{k}_std"] = query_stability_df[f"recall_at_{k}_std"].mean()
                final_metrics[f"rnk_stab_mean_jaccard_at_{k}"] = query_stability_df[f"jaccard_at_{k}_mean_vs_first_loop"].mean()
        ...
        self._calculate_stability_score(cleaned_metrics, query_stability_df if query_stability_metrics else pl.DataFrame())

        return EvaluationResult(metrics=cleaned_metrics, plots={}, extra_data=extra_data)
    ...
    def _calculate_stability_score(self, metrics: Dict[str, Any], details_df: pl.DataFrame):
        ...
        k_for_scoring = 10
        if 10 not in self.k_values and self.k_values: # Fallback to largest K if 10 not present
        ...
        score_ndcg_stability = normalize_metric_to_score(
            ...
            good_threshold=0.05, bad_threshold=0.2, lower_is_better=True # Example thresholds for std dev
        )
        if score_ndcg_stability is None: score_ndcg_stability = 0.0

        score_recall_stability = normalize_metric_to_score(
            metrics.get(f"rnk_stab_mean_recall_at_{k_for_scoring}_std"),
            good_threshold=0.05, bad_threshold=0.2, lower_is_better=True
        )
        if score_recall_stability is None: score_recall_stability = 0.0
        ...
        if score_jaccard is None: score_jaccard = 0.0
        ...
        total_w = sum(weights.get(k,0.0) for k in ["ndcg_stability", "recall_stability", "top_k_set_jaccard"])
        if total_w > 1e-6 and abs(total_w - 1.0) > 1e-6: s_stability_ranking /= total_w

        metrics[f"rnk_stab_score_ndcg_at_{k_for_scoring}_stability"] = score_ndcg_stability
        metrics[f"rnk_stab_score_recall_at_{k_for_scoring}_stability"] = score_recall_stability
        ...
        logger.info(f"Ranking Stability Score (Overall): {metrics['rnk_stab_score_overall']:.2f}")
...
"""
    ...
    print("\nRanking Stability Metrics:", {k:v for k,v in results.metrics.items() if v is not None})
    ...
    if "ranking_stability_details_per_query_df" in results.extra_data:
        print("\nDetails per query:", results.extra_data["ranking_stability_details_per_query_df"])
        assert results.extra_data["ranking_stability_details_per_query_df"].shape[0] == 2 # 2 queries

    if dummy_path.exists(): dummy_path.unlink()
    import shutil
    if Path("./test_rank_stab_output").exists(): shutil.rmtree("./test_rank_stab_output")
