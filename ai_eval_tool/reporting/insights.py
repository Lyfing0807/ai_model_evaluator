...
from ..config_manager import (  # For API key, endpoint, model
    AIInsightsConfig,
    MainConfig,
)
...
class AIInsightsGenerator:
    ...
    def __init__(self, insights_config: AIInsightsConfig, project_config: MainConfig):
        ...
        self.cache_dir = (
            Path(project_config.report_settings.output_dir) / ".insights_cache"
        )
        if self.config.cache_responses:
            self.cache_dir.mkdir(parents=True, exist_ok=True)

        logger.info(
            f"AIInsightsGenerator initialized. Caching enabled: {self.config.cache_responses}"
        )
    ...
    def _get_api_key(self) -> Optional[str]:
        ...
        api_key_env_var = getattr(
            self.insights_config, "api_key_env_var", "OPENAI_API_KEY"
        )
    ...
    def _build_prompt(
        ...
        ideal_description: Optional[str] = None,
    ) -> str:
        """
        ...
        if ideal_description:
            prompt_lines.append(
                f"Ideal characteristics for this chart: {ideal_description}"
            )
        ...
        prompt = "\n".join(prompt_lines)
        logger.debug(
            f"Generated LLM Prompt for {chart_type} (first 200 chars):\n{prompt[:200]}..."
        )
        return prompt
    ...
    def _make_llm_api_call(self, prompt: str) -> Optional[str]:
        ...
        payload = {
            ...
            or "gpt-3.5-turbo",  # Default if not specified
        ...
        logger.info(
            f"Sending request to LLM API: {self.config.api_endpoint} with model: {payload['model']}"
        )
        try:
            ...
            logger.error(
                f"LLM API returned an error: {e.response.status_code} - {e.response.text}"
            )
        except json.JSONDecodeError:
            logger.error(
                f"Failed to decode JSON response from LLM API: {response.text}"
            )
        except Exception as e:
            logger.error(
                f"An unexpected error occurred during LLM API call: {e}", exc_info=True
            )
        return None
    ...
    def _generate_cache_key(
        ...
        hasher.update(
            chart_data_summary.encode()
        )  # Ensure chart_data_summary is a stable string representation
    ...
    def get_insight_for_chart(
        ...
        cache_key = self._generate_cache_key(
            chart_type, chart_data_summary, context_description
        )
        ...
        if self.config.cache_responses and cache_file.exists():
            try:
                ...
                logger.warning(
                    f"Failed to read insight from cache file {cache_file}: {e}"
                )
        ...
        if insight_text and self.config.cache_responses:
            try:
                ...
                logger.warning(
                    f"Failed to write insight to cache file {cache_file}: {e}"
                )
...
if __name__ == "__main__":
    ...
    if not os.getenv("OPENAI_API_KEY"):
        logger.warning(
            "OPENAI_API_KEY not set. AI insight generation test will skip actual API call if enabled=True."
        )
        # insights_cfg.enabled = False # Optionally disable for test if key is missing

    ai_gen = AIInsightsGenerator(
        insights_config=insights_cfg, project_config=project_cfg
    )
    ...
    if insight:
        ...
        logger.warning(
            "Failed to generate insight (as expected if API key is missing or disabled)."
        )
