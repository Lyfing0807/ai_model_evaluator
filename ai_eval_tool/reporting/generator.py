...
from ..config_manager import MainConfig, ReportSettings
...
try:
    ...
    try:
        ...
        logger.warning(
            "DejaVuSans.ttf or Arial.ttf not found. Using Pillow's default font."
        )
    ...
    if label:
        try:  # textbbox can fail with some fonts / PIL versions if text is empty or very small
            text_bbox = draw.textbbox(
                (xmin, ymin - 20 if ymin - 20 > 0 else ymin), label, font=FONT
            )
            draw.rectangle(text_bbox, fill=color)
            draw.text(
                (xmin, ymin - 20 if ymin - 20 > 0 else ymin),
                label,
                fill="white",
                font=FONT,
            )
...
class ReportGenerator:
    def __init__(self, config: MainConfig):
        ...
        if self.report_settings.ai_insights.enabled:
            try:
                ...
                logger.error(
                    f"Failed to initialize AIInsightsGenerator: {e}", exc_info=True
                )
    ...
    def _save_chart_mpl(
        self, fig: plt.Figure, chart_name: str, run_id: str, charts_subdir: Path
    ) -> Path:
        ...
        try:
            ...
            logger.error(
                f"Failed to save Matplotlib chart {chart_name} to {chart_path_abs}: {e}"
            )
            plt.close(fig)
    ...
    def _get_plotly_div(self, fig_generator, *args, **kwargs) -> str:
        try:
            ...
            logger.error(
                f"Failed to generate Plotly div for {fig_generator.__name__}: {e}",
                exc_info=True,
            )
    ...
    def _resolve_image_path(self, relative_path_str: Optional[str]) -> Optional[Path]:
        ...
        try:
            ...
            logger.debug(
                f"Could not resolve image path: {relative_path_str} (base_dir: {self.image_base_dir})"
            )
            return None
    ...
    def _process_image_for_report(
        self,
        resolved_img_path: Path,
        row_dict: Dict[str, Any],
        sample_index: int,
        run_id: str,
        annotated_img_subdir: Path,
        ...
        try:
            ...
            img_name_slug = self._generate_safe_filename(
                original_path_str, sample_index
            )
            loop_id = row_dict.get("loop", row_dict.get("loop_id", "L"))
            image_id_val = row_dict.get(
                "image_id", row_dict.get("img_name", f"ID{sample_index}")
            )
            ...
            logger.error(
                f"Error processing image {resolved_img_path} for report: {e}",
                exc_info=True,
            )
            return None, None

    def _generate_safe_filename(
        self, original_path_str: Optional[str], sample_index: int
    ) -> str:
        """Generate a safe filename component from the original path."""
        if original_path_str:
            try:
                return Path(original_path_str).stem.replace(" ", "_")[
                    :50
                ]  # Limit length
            except Exception:
                pass
    ...
    def _create_thumbnail(
        self, source_path: Path, base_filename: str, output_dir: Path, run_id: str
    ) -> Optional[Path]:
    ...
    def _create_lightbox_image(
        ...
        try:
            if (
                self.report_settings.display_images.draw_annotations
                and self.project_info.model_type
                in ["detection", "pose", "rotated_detection"]
            ):
    ...
    def _create_annotated_image(
        ...
        try:
            ...
            with Image.open(source_path).convert("RGBA") as img:
                ...
                if (
                    self.project_info.model_type == "detection"
                    and "internal_bbox" in row_dict
                ):
    ...
    def _copy_original_image(
        ...
        try:
            ...
            original_suffix = (
                Path(original_path_str).suffix if original_path_str else ".png"
            )
            ...
            if original_suffix.lower() in [".png", ".jpg", ".jpeg"]:
                ...
                with Image.open(source_path) as img:
                    ...
                    img.save(
                        copied_abs_path,
                        format="PNG" if original_suffix.lower() == ".png" else "JPEG",
                        optimize=True,
                        quality=85,
                    )
            else:
    ...
    def _prepare_abnormal_samples_display_data(
        self,
        df: pl.DataFrame,
        category_name: str,
        sort_by_col: str,
        ...
        if (
            df.is_empty()
            or sort_by_col not in df.columns
            or "image_path" not in df.columns
        ):
            logger.warning(
                f"Cannot prepare abnormal samples for '{category_name}': DataFrame empty or missing required columns (sort_by_col:'{sort_by_col}', 'image_path')."
            )
            return display_data
        sorted_df = df.sort(sort_by_col, descending=not sort_ascending)
        samples_to_display = sorted_df.head(
            self.report_settings.display_images.max_per_category
        )

        for i, row_dict in enumerate(samples_to_display.to_dicts()):
            original_img_path_str = row_dict.get(
                "image_path"
            )  # From config field_mapping
            ...
            if resolved_original_img_path:
                thumbnail_rel_path, lightbox_rel_path = self._process_image_for_report(
                    resolved_original_img_path,
                    row_dict,
                    i,
                    run_id,
                    annotated_img_subdir,
                    original_img_path_str,
                )
            else:
                logger.warning(
                    f"Could not resolve image path '{original_img_path_str}' (base dir: {self.image_base_dir})."
                )

            sample_info["thumbnail_path"] = (
                str(thumbnail_rel_path) if thumbnail_rel_path else None
            )
            sample_info["lightbox_image_path"] = (
                str(lightbox_rel_path)
                if lightbox_rel_path
                else (str(thumbnail_rel_path) if thumbnail_rel_path else None)
            )
            display_data.append(sample_info)
        return display_data

    def _prepare_report_context(
        self, eval_results: EvaluationResult, run_id: str
    ) -> Dict[str, Any]:
        context: Dict[str, Any] = {
            ...
            "config_yaml": yaml.dump(
                self.main_config.model_dump(exclude_none=True),
                indent=2,
                sort_keys=False,
            ),
        ...
        if perf_df is not None and not perf_df.is_empty():
            ...
            context["md_charts_paths"]["latency_distribution"] = self._save_chart_mpl(
                chart_generators.generate_latency_distribution_plot_mpl(perf_df),
                "latency_distribution",
                run_id,
                charts_subdir,
            )
            latency_plotly_div = self._get_plotly_div(
                chart_generators.generate_latency_distribution_plotly, perf_df
            )
            latency_ai_insight = None
            if self.ai_insights_generator:
                latency_summary = (
                    perf_df.select(pl.col("total_time_ms")).describe().to_dicts()
                )
                latency_ai_insight = self.ai_insights_generator.get_insight_for_chart(
                    "Latency Distribution",
                    latency_summary,
                    f"Model: {self.project_info.model_name or 'N/A'}",
                )
            context["plotly_charts"]["latency_distribution"] = {
                "html_div": latency_plotly_div,
                "ai_insight": latency_ai_insight,
            }

            # Performance Timeline
            context["md_charts_paths"]["performance_timeline"] = self._save_chart_mpl(
                chart_generators.generate_performance_timeline_plot_mpl(perf_df),
                "performance_timeline",
                run_id,
                charts_subdir,
            )
            context["plotly_charts"]["performance_timeline"] = {
                "html_div": self._get_plotly_div(
                    chart_generators.generate_performance_timeline_plotly, perf_df
                )
            }  # AI insight can be added later

        if eval_results.metrics:  # Time Composition
            context["md_charts_paths"]["time_composition"] = self._save_chart_mpl(
                chart_generators.generate_time_composition_plot_mpl(
            ...
            context["plotly_charts"]["time_composition"] = {
                "html_div": self._get_plotly_div(
                    chart_generators.generate_time_composition_plotly,
                    eval_results.metrics,
                )
            }
        ...
        if model_type == "detection":
            ...
            if obj_stab_df is not None and not obj_stab_df.is_empty():
                context["md_charts_paths"]["iou_consistency_distribution"] = (
                    self._save_chart_mpl(
                        chart_generators.generate_iou_consistency_distribution_plot_mpl(
                            obj_stab_df
                        ),
                        "iou_consistency_distribution",
                        run_id,
                        charts_subdir,
                    )
                )
                iou_plotly_div = self._get_plotly_div(
                    chart_generators.generate_iou_consistency_distribution_plotly,
                    obj_stab_df,
                )
                iou_ai_insight = None
                if self.ai_insights_generator:
                    iou_summary = (
                        obj_stab_df.select(pl.col("mean_iou_consistency"))
                        .describe()
                        .to_dicts()
                    )
                    iou_ai_insight = self.ai_insights_generator.get_insight_for_chart(
                        "IoU Consistency Distribution",
                        iou_summary,
                        "Detection model IoU stability.",
                    )
                context["plotly_charts"]["iou_consistency_distribution"] = {
                    "html_div": iou_plotly_div,
                    "ai_insight": iou_ai_insight,
                }

                context["md_charts_paths"]["center_drift_distribution"] = (
                    self._save_chart_mpl(
                        chart_generators.generate_center_drift_distribution_plot_mpl(
                            obj_stab_df
                        ),
                        "center_drift_distribution",
                        run_id,
                        charts_subdir,
                    )
                )
                context["plotly_charts"]["center_drift_distribution"] = {
                    "html_div": self._get_plotly_div(
                        chart_generators.generate_center_drift_distribution_plotly,
                        obj_stab_df,
                    )
                }

        elif model_type == "classification":
            cls_stab_df = eval_results.extra_data.get(
                "classification_stability_details_df"
            )
            if cls_stab_df is not None and not cls_stab_df.is_empty():
                context["plotly_charts"]["top_1_consistency_distribution"] = {
                    "html_div": self._get_plotly_div(
                        chart_generators.generate_top_1_consistency_distribution_plotly,
                        cls_stab_df,
                    )
                }
                context["plotly_charts"]["confidence_std_distribution"] = {
                    "html_div": self._get_plotly_div(
                        chart_generators.generate_confidence_std_distribution_plotly,
                        cls_stab_df,
                    )
                }
            cls_eval_params = self.main_config.evaluation_params.classification
            if cls_eval_params and eval_results.metrics:
                context["plotly_charts"]["jaccard_similarity_trends"] = {
                    "html_div": self._get_plotly_div(
                        chart_generators.generate_jaccard_similarity_trends_plotly,
                        eval_results.metrics,
                        cls_eval_params.top_k,
                    )
                }
            ...
            if isinstance(data_item, pl.DataFrame):
                try:
                    if data_item.height > 0:
                        csv_fn = f"{key}.csv"
                        csv_abs = extra_data_subdir / csv_fn
                        data_item.write_csv(csv_abs)
                        context["extra_data_csv_paths"][key] = (
                            Path(run_id) / "data" / csv_fn
                        )
                        context["extra_data_html"][key] = (
                            data_item.head(15)
                            .to_pandas()
                            .to_html(
                        ...
                        context["extra_data_html"][
                            key
                        ] = "<p><i>DataFrame is empty.</i></p>"
                except Exception as e:
                    logger.error(f"Failed to process DataFrame {key}: {e}")
                    context["extra_data_html"][
                        key
                    ] = f"<p><i>Error processing DataFrame {key}.</i></p>"

        if self.report_settings.display_images.enabled:
            slow_samples_df = eval_results.extra_data.get("perf_top_slow_samples_df")
            if slow_samples_df is not None and isinstance(
                slow_samples_df, pl.DataFrame
            ):
                ...
                img_path_col_name = (
                    self.main_config.data_loader.field_mapping.image_path
                )
                if (
                    img_path_col_name not in slow_samples_df.columns
                    and "image_id" in slow_samples_df.columns
                    and perf_df is not None
                ):
                    # Attempt to join image_path from the deduplicated perf_df
                    slow_samples_df = slow_samples_df.join(
                        perf_df.select(["image_id", "loop", img_path_col_name]).unique(
                            subset=["image_id", "loop"]
                        ),
                        on=["image_id", "loop"],
                        how="left",
                    ).rename(
                        {img_path_col_name: "image_path"}
                    )  # Standardize to "image_path" for _prepare_abnormal_samples

                if "image_path" in slow_samples_df.columns:
                    context["abnormal_image_samples_by_category"][
                        "Performance: Top Slow Samples"
                    ] = self._prepare_abnormal_samples_display_data(
                        slow_samples_df,
                        "PerfSlow",
                        "total_time_ms",
                        False,
                    ...
                    logger.warning(
                        "'image_path' column not found in 'perf_top_slow_samples_df'. Cannot display abnormal images for performance."
                    )
    ...
    def _generate_md_report(self, context: Dict[str, Any], run_id: str):
        ...
        score_metrics = {
            k: v for k, v in context.get("metrics", {}).items() if "score" in k.lower()
        }
        if score_metrics:
            ...
            for score_key in [
                ...
                if score_key in score_metrics:
                    val = score_metrics[score_key]
                    val_s = f"{val:.2f}" if isinstance(val, float) else str(val)
                    title = (
                        score_key.replace("_score_overall", "")
                        .replace("_", " ")
                        .title()
                    )
                    if score_key == "total_score_overall":
                        title += " (Perf & Stab)"
        ...
        if context["metrics"]:
            ...
            perf_m = {
                ...
                if k.startswith("perf_") and "score" not in k
            }
            stab_m = {
                k: v
                for k, v in context["metrics"].items()
                if (
                    k.startswith(model_stab_prefix_short)
                    or k.startswith(model_stab_prefix_det)
                )
                and "score" not in k
            }
            other_m = {
                ...
                if not (
                    k.startswith("perf_")
                    or k.startswith(model_stab_prefix_short)
                    or k.startswith(model_stab_prefix_det)
                    or "score" in k.lower()
                )
            }
            ...
            def format_metrics_md(title, metrics_dict, prefix_to_strip=""):
                s = f"### {title}\n"
                if not metrics_dict:
                    return s + "_No metrics in this category._\n\n"
                for k, v in metrics_dict.items():
                    val_str = f"{v:.4f}" if isinstance(v, float) else str(v)
                    val_str = "N/A" if v is None else val_str
                    disp_k = k.replace(prefix_to_strip, "", 1).replace("_", " ").title()
                    s += f"- **{disp_k}**: `{val_str}`\n"
                return s + "\n"

            content += format_metrics_md("Performance Metrics", perf_m, "perf_")
            content += format_metrics_md(
                f"{context['model_type'].title()} Stability Metrics",
                stab_m,
                (
                    model_stab_prefix_short
                    if context["model_type"] != "detection"
                    else model_stab_prefix_det
                ),
            )
            if other_m:
                content += format_metrics_md("Other Metrics", other_m)
        else:
        ...
        if context["md_charts_paths"]:
            ...
            content += "_No charts were generated._\n\n"
        ...
        if context["extra_data_csv_paths"]:
            ...
            content += "_No additional data files generated._\n\n"
        ...
        try:
            ...
            logger.error(f"Failed to write MD report {report_path}: {e}")
    ...
    def _generate_html_report(self, context: Dict[str, Any], run_id: str):
        ...
        html_context["static_charts_for_html"] = {
            name: {"title": name.replace("_", " ").title(), "path": str(path_obj)}
            for name, path_obj in context.get("md_charts_paths", {}).items()
        }
        ...
        try:
            ...
            logger.error(
                f"Failed to generate HTML report {report_path}: {e}", exc_info=True
            )
    ...
    def generate(self, eval_results: EvaluationResult, run_id: str):
        ...
        logger.info(
            f"Report generation completed. Reports saved in: {self.report_settings.output_dir.resolve()}"
        )
...
"""
    ...
    try:
        ...
        img_draw.text((10, 10), "Sample1", fill="black", font=FONT)
        img.save(Path("./test_report_images/sample1.png"))
        img = Image.new("RGB", (180, 120), color="lightgreen")
        img_draw = ImageDraw.Draw(img)
        img_draw.text((10, 10), "Sample2", fill="black", font=FONT)
        img.save(Path("./test_report_images/sample2.png"))
    except Exception as e:
        logger.warning(f"Could not create dummy images: {e}")
    ...
    if not (template_dir_for_test / "report_template.html").exists():
        (template_dir_for_test / "report_template.html").write_text(
            "<html><body><h1>Test Report {{run_id}}</h1></body></html>"
        )
    ...
    try:
        ...
        dummy_perf_df = pl.DataFrame(
            {
                "total_time_ms": [100, 250],
                "_event_index": range(2),
                "loop": [1, 1],
                "image_id": ["s1", "s2"],
                "image_path_col": ["sample1.png", "sample2.png"],
                "internal_bbox": [[[10, 10, 50, 50]], [[20, 20, 60, 60]]],
                "cat_col": [0, 1],
                "score_col": [0.9, 0.8],
            }
        )
        dummy_obj_stab_df = pl.DataFrame(
            {"mean_iou_consistency": [0.8], "mean_center_drift_px": [5.0]}
        )

        dummy_eval_results = EvaluationResult(
            metrics={
                "perf_mean_total_time_ms": 106.0,
                "det_stab_mean_iou_consistency": 0.81,
            },
            extra_data={
                ...
                "perf_top_slow_samples_df": dummy_perf_df.sort(
                    "total_time_ms", descending=True
                ),  # Has image_path and bbox
            },
        )
        ...
        assert (
            run_out_dir / "charts" / "annotated_images" / "thumb_sample1_1_s1.png"
        ).exists()  # Check one thumb
        logger.info(f"Test reports generated in: {run_out_dir.resolve()}")
    except Exception as e:
        logger.error(f"Error during ReportGenerator test: {e}", exc_info=True)
        raise
    finally:
        if dummy_config_path.exists():
            dummy_config_path.unlink()
        if Path("./test_report_images").exists():
            shutil.rmtree("./test_report_images")
