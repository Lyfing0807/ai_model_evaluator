...
def generate_latency_distribution_plot_mpl(
    perf_df: pl.DataFrame, time_col: str = "total_time_ms", title: Optional[str] = None
) -> plt.Figure:
    fig, ax = plt.subplots(figsize=(10, 6))
    data_pd = perf_df.select(pl.col(time_col).drop_nulls()).to_pandas()
    if data_pd.empty or data_pd[time_col].isnull().all():
        logger.warning(
            f"No valid data for Matplotlib latency distribution (column: {time_col})."
        )
        ax.text(0.5, 0.5, "No data available", ha="center", va="center", fontsize=12)
        return fig
    sns.violinplot(y=data_pd[time_col], ax=ax, inner="quartile", color="skyblue", cut=0)
    ax.set_title(
        title or f"{time_col.replace('_', ' ').title()} Distribution", fontsize=15
    )
...
def generate_performance_timeline_plot_mpl(
    perf_df: pl.DataFrame, time_col: str = "total_time_ms", title: Optional[str] = None
) -> plt.Figure:
    # Implementation remains the same
    fig, ax = plt.subplots(figsize=(12, 6))
    plot_df = (
        perf_df.with_row_count("_event_index")
        if "_event_index" not in perf_df.columns
        else perf_df
    )
    data_pd = plot_df.select(
        ["_event_index", pl.col(time_col).drop_nulls()]
    ).to_pandas()
    if data_pd.empty or data_pd[time_col].isnull().all():
        ax.text(0.5, 0.5, "No data available", ha="center", va="center", fontsize=12)
        return fig
    ...
    ax.set_title(
        title or f"{time_col.replace('_', ' ').title()} Over Time", fontsize=15
    )
...
def generate_iou_consistency_distribution_plot_mpl(
    stability_details_df: pl.DataFrame,
    iou_col: str = "mean_iou_consistency",
    ...
    if data_pd.empty or data_pd[iou_col].isnull().all():
        ax.text(
            0.5,
            0.5,
            "No valid data for IoU consistency",
            ha="center",
            va="center",
            fontsize=12,
        )
...
def generate_center_drift_distribution_plot_mpl(
    stability_details_df: pl.DataFrame,
    drift_col: str = "mean_center_drift_px",
    title: Optional[str] = None,
) -> plt.Figure:
    # Implementation remains the same
    fig, ax = plt.subplots(figsize=(10, 6))
    if stability_details_df.is_empty() or drift_col not in stability_details_df.columns:
        ax.text(
            0.5, 0.5, "No data for center drift", ha="center", va="center", fontsize=12
        )
        return fig
    ...
    if data_pd.empty or data_pd[drift_col].isnull().all():
        ax.text(
            0.5,
            0.5,
            "No valid data for center drift",
            ha="center",
            va="center",
            fontsize=12,
        )
...
def generate_latency_distribution_plotly(
    perf_df: pl.DataFrame, time_col: str = "total_time_ms", title: Optional[str] = None
) -> go.Figure:
    # Implementation remains the same
    data_pd = perf_df.select(pl.col(time_col).drop_nulls()).to_pandas()
    ...
    fig.update_layout(
        title_text=title
        or f"{time_col.replace('_', ' ').title()} Distribution (Interactive)",
        yaxis_title="Time (ms)",
    )
    return fig
...
def generate_time_composition_plotly(metrics: Dict[str, Any]) -> go.Figure:
    ...
    if not valid_components:
        return go.Figure().add_annotation(
            text="No data for time composition",
            xref="paper",
            yref="paper",
            showarrow=False,
            font_size=12,
    ...
    fig = px.bar(
        x=list(valid_components.keys()),
        y=list(valid_components.values()),
        labels={"x": "Processing Stage", "y": "Average Time (ms)"},
        text_auto=".2f",
    )
...
def generate_performance_timeline_plotly(
    perf_df: pl.DataFrame, time_col: str = "total_time_ms", title: Optional[str] = None
) -> go.Figure:
    plot_df = (
        perf_df.with_row_count("_event_index")
    ...
    fig = px.scatter(
        ...
        x="_event_index",
        y=time_col,
        title=title or f"{time_col.replace('_', ' ').title()} Over Time (Interactive)",
    )
    fig.update_layout(xaxis_title="Event Index (Sequence)", yaxis_title="Time (ms)")
    return fig


def generate_iou_consistency_distribution_plotly(
    stability_details_df: pl.DataFrame,
    iou_col: str = "mean_iou_consistency",
    title: Optional[str] = None,
) -> go.Figure:
    if stability_details_df.is_empty() or iou_col not in stability_details_df.columns:
        return go.Figure().add_annotation(
            text="No data for IoU consistency",
            xref="paper",
            yref="paper",
            showarrow=False,
            font_size=12,
    ...
    if data.is_empty() or data[iou_col].null_count() == data.height:
        return go.Figure().add_annotation(
            text="No valid data for IoU consistency",
            xref="paper",
            yref="paper",
            showarrow=False,
            font_size=12,
    ...
    fig = px.histogram(
        ...
        x=iou_col,
        marginal="box",
        title=title or "Distribution of Mean IoU Consistency (Interactive)",
        nbins=30,
    )
    fig.update_layout(
        xaxis_title="Mean IoU Consistency", yaxis_title="Number of Tracked Objects"
    )
    return fig


def generate_center_drift_distribution_plotly(
    stability_details_df: pl.DataFrame,
    drift_col: str = "mean_center_drift_px",
    title: Optional[str] = None,
) -> go.Figure:
    ...
    if data.is_empty() or data[drift_col].null_count() == data.height:
        return go.Figure().add_annotation(
            text="No valid data for center drift",
            xref="paper",
            yref="paper",
            showarrow=False,
            font_size=12,
    ...
    fig = px.histogram(
        ...
        x=drift_col,
        marginal="box",
        title=title or "Distribution of Mean Center Point Drift (Interactive)",
        nbins=30,
    )
    fig.update_layout(
        xaxis_title="Mean Center Drift (pixels)",
...
def generate_top_1_consistency_distribution_plotly(
    cls_stability_df: pl.DataFrame,
    consistency_col: str = "top_1_consistency_rate",
    title: Optional[str] = None,
) -> go.Figure:
    if cls_stability_df.is_empty() or consistency_col not in cls_stability_df.columns:
        return go.Figure().add_annotation(
            text="No data for Top-1 Consistency",
            xref="paper",
            yref="paper",
            showarrow=False,
            font_size=12,
    ...
    if data.is_empty() or data[consistency_col].null_count() == data.height:
        return go.Figure().add_annotation(
            text="No valid data for Top-1 Consistency",
            xref="paper",
            yref="paper",
            showarrow=False,
            font_size=12,
    ...
    fig = px.histogram(
        data.to_pandas(),
        x=consistency_col,
        title=title or "Distribution of Top-1 Consistency Rate per Image (Interactive)",
        nbins=20,
        range_x=[0, 1],
    )
    fig.update_layout(
        xaxis_title="Top-1 Consistency Rate", yaxis_title="Number of Images"
    )
    return fig


def generate_jaccard_similarity_trends_plotly(
    metrics: Dict[str, Any], k_values: List[int], title: Optional[str] = None
) -> go.Figure:
    jaccard_data = []
    for k in k_values:
        ...
        if metric_name in metrics and metrics[metric_name] is not None:
            jaccard_data.append(
                {"K": f"Top-{k}", "Mean Jaccard Similarity": metrics[metric_name]}
            )

    if not jaccard_data:
        return go.Figure().add_annotation(
            text="No Jaccard similarity data available",
            xref="paper",
            yref="paper",
            showarrow=False,
            font_size=12,
...
def generate_confidence_std_distribution_plotly(
    cls_stability_df: pl.DataFrame,
    conf_std_col: str = "top_1_confidence_std",
    title: Optional[str] = None,
) -> go.Figure:
    if cls_stability_df.is_empty() or conf_std_col not in cls_stability_df.columns:
        return go.Figure().add_annotation(
            text="No data for Confidence Std Dev",
            xref="paper",
            yref="paper",
            showarrow=False,
            font_size=12,
    ...
    if data.is_empty() or data[conf_std_col].null_count() == data.height:
        return go.Figure().add_annotation(
            text="No valid data for Confidence Std Dev",
            xref="paper",
            yref="paper",
            showarrow=False,
            font_size=12,
    ...
    fig = px.histogram(
        ...
        x=conf_std_col,
        title=title
        or "Distribution of Top-1 Confidence Std Dev per Image (Interactive)",
        nbins=20,
    )
    fig.update_layout(
        xaxis_title="Top-1 Confidence Std Dev", yaxis_title="Number of Images"
    )
    return fig
...
if __name__ == "__main__":
    ...
    deduplicated_perf_df = dummy_perf_df_main.unique(
        subset=["total_time_ms"], keep="first"
    )
    ...
    def save_test_fig_mpl(fig: plt.Figure, name: str):
        try:
            path = test_charts_dir / f"{name}_mpl.png"
            fig.savefig(path)
            logger.info(f"Saved MPL chart: {path}")
        except Exception as e:
            logger.error(f"Failed to save {name}_mpl.png: {e}")
        finally:
            plt.close(fig)

    def save_test_fig_plotly(fig: go.Figure, name: str):
        try:
            path = test_charts_dir / f"{name}_plotly.html"
            fig.write_html(path)
            logger.info(f"Saved Plotly chart: {path}")
        except Exception as e:
            logger.error(f"Failed to save {name}_plotly.html: {e}")

    # Test performance charts
    save_test_fig_mpl(
        generate_latency_distribution_plot_mpl(deduplicated_perf_df), "latency_dist"
    )
    save_test_fig_plotly(
        generate_latency_distribution_plotly(deduplicated_perf_df), "latency_dist"
    )

    save_test_fig_mpl(
        generate_time_composition_plot_mpl(dummy_metrics_main), "time_comp"
    )
    save_test_fig_plotly(
        generate_time_composition_plotly(dummy_metrics_main), "time_comp"
    )

    save_test_fig_mpl(
        generate_performance_timeline_plot_mpl(deduplicated_perf_df), "perf_timeline"
    )
    save_test_fig_plotly(
        generate_performance_timeline_plotly(deduplicated_perf_df), "perf_timeline"
    )

    # Test detection stability charts
    save_test_fig_mpl(
        generate_iou_consistency_distribution_plot_mpl(dummy_det_stability_df),
        "iou_consistency_dist",
    )
    save_test_fig_plotly(
        generate_iou_consistency_distribution_plotly(dummy_det_stability_df),
        "iou_consistency_dist",
    )

    save_test_fig_mpl(
        generate_center_drift_distribution_plot_mpl(dummy_det_stability_df),
        "center_drift_dist",
    )
    save_test_fig_plotly(
        generate_center_drift_distribution_plotly(dummy_det_stability_df),
        "center_drift_dist",
    )

    # Test classification stability charts (Plotly only as per current additions)
    save_test_fig_plotly(
        generate_top_1_consistency_distribution_plotly(dummy_cls_stability_df),
        "cls_top1_consistency_dist",
    )
    save_test_fig_plotly(
        generate_jaccard_similarity_trends_plotly(
            dummy_metrics_main, k_values=[1, 3, 5]
        ),
        "cls_jaccard_trends",
    )
    save_test_fig_plotly(
        generate_confidence_std_distribution_plotly(dummy_cls_stability_df),
        "cls_conf_std_dist",
    )

    logger.info(
        f"Chart generation function tests completed. Check figs in {test_charts_dir.resolve()}"
    )
