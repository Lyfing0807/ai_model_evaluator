...
def run_cli_command(args_list):
    ...
    return subprocess.run(command, capture_output=True, text=True, check=False) # check=False to handle non-zero exits
...
def test_cli_missing_required_args():
    ...
    assert "error: the following arguments are required: --config" in result.stderr.lower() # argparse error

    result = run_cli_command(["--config", "dummy.yaml"]) # Missing --csv
    assert result.returncode != 0
    assert "error: --csv is required if not using --report-only" in result.stderr.lower()


def test_cli_full_run_detection(dummy_detection_config_path: Path, dummy_detection_csv_path: Path, test_data_dir: Path):
    ...
    report_output_dir_name = Path(config_data["report_settings"]["output_dir"]).name # e.g. "reports_detection"
    report_output_dir = test_data_dir / report_output_dir_name # This is where reports will go

    # The cache will be under report_output_dir / ".cache" / run_id
    # Logs will be under report_output_dir / run_id / "logs"

    args = [
        "--config", str(dummy_detection_config_path),
        "--csv", str(dummy_detection_csv_path),
        "--log-level", "DEBUG"
    ]
    ...
    assert result.returncode == 0
    assert "AI Model Evaluation process completed successfully" in result.stdout # Check for success message
...
def test_cli_analyze_only_mode(dummy_classification_config_path: Path, dummy_classification_csv_path: Path, test_data_dir: Path):
    ...
    args = [
        "--config", str(dummy_classification_config_path),
        "--csv", str(dummy_classification_csv_path),
        "--analyze-only"
    ]
    result = run_cli_command(args)
    ...
    assert (expected_run_cache_dir / "extra_data_deduplicated_perf_df_for_charts.parquet").exists()
    # Classification stability details df should also be cached
    assert (expected_run_cache_dir / "extra_data_classification_stability_details_df.parquet").exists()


def test_cli_report_only_mode(dummy_classification_config_path: Path, dummy_classification_csv_path: Path, test_data_dir: Path):
    ...
    cache_base_dir = report_output_dir / ".cache"
    run_cache_dir_path_str = str(cache_base_dir / run_id) # Path to the specific run's cache
    ...
    assert analyze_result.returncode == 0, f"Analyze-only pre-step failed: {analyze_result.stderr}"

    # Now run report-only using the created cache
    report_args = [
        "--config", str(dummy_classification_config_path), # Config still needed for report settings
        "--report-only",
        "--cache-path", run_cache_dir_path_str # Provide path to the run_id specific cache dir
    ]
...
def test_cli_report_only_missing_cache(dummy_classification_config_path: Path, test_data_dir: Path):
    # Try to run report-only with a cache path that doesn't exist
    non_existent_cache_path = str(test_data_dir / ".cache" / "non_existent_run_id_for_report")

    args = [
        "--config", str(dummy_classification_config_path),
        "--report-only",
        "--cache-path", non_existent_cache_path
    ]
    result = run_cli_command(args)
    assert result.returncode != 0
