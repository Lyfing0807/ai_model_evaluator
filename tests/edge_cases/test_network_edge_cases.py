...
class TestOpenAIAPIFailures:
    ...
    def test_openai_api_timeout(self, mock_openai, main_config_with_ai, sample_evaluation_result):
    ...
    @patch('ai_eval_tool.reporting.insights.openai')
    def test_openai_api_connection_error(self, mock_openai, main_config_with_ai, sample_evaluation_result):
        ...
        generator = AIInsightsGenerator(main_config_with_ai)
        insights = generator.generate_insights(sample_evaluation_result, "connection-test")
        
        assert insights == ""
        mock_openai.ChatCompletion.create.assert_called_once()
    
    @patch('ai_eval_tool.reporting.insights.openai')
    def test_openai_api_rate_limit(self, mock_openai, main_config_with_ai, sample_evaluation_result):
        """Test OpenAI API rate limiting."""
        # Mock rate limit error
        mock_openai.ChatCompletion.create.side_effect = Exception("Rate limit exceeded")
        
        generator = AIInsightsGenerator(main_config_with_ai)
        insights = generator.generate_insights(sample_evaluation_result, "rate-limit-test")
        
        assert insights == ""
    
    @patch('ai_eval_tool.reporting.insights.openai')
    def test_openai_api_invalid_key(self, mock_openai, main_config_with_ai, sample_evaluation_result):
        """Test OpenAI API with invalid API key."""
        # Mock authentication error
        mock_openai.ChatCompletion.create.side_effect = Exception("Invalid API key")
        
        generator = AIInsightsGenerator(main_config_with_ai)
        insights = generator.generate_insights(sample_evaluation_result, "invalid-key-test")
        
        assert insights == ""
    
    @patch('ai_eval_tool.reporting.insights.openai')
    def test_openai_api_malformed_response(self, mock_openai, main_config_with_ai, sample_evaluation_result):
        ...
        generator = AIInsightsGenerator(main_config_with_ai)
        insights = generator.generate_insights(sample_evaluation_result, "malformed-response-test")
        
        # Should handle empty choices gracefully
        assert insights == ""
    
    @patch('ai_eval_tool.reporting.insights.openai')
    def test_openai_api_partial_response(self, mock_openai, main_config_with_ai, sample_evaluation_result):
        ...
        generator = AIInsightsGenerator(main_config_with_ai)
        insights = generator.generate_insights(sample_evaluation_result, "partial-response-test")
...
class TestCustomAPIFailures:
    ...
    def test_custom_api_timeout(self, mock_post, main_config_with_ai, sample_evaluation_result):
        """Test custom API timeout."""
        # Configure for custom API
        main_config_with_ai.ai_insights.provider = "custom"
        main_config_with_ai.ai_insights.api_endpoint = "https://custom-api.example.com/chat"
        
        # Mock timeout
        mock_post.side_effect = requests.exceptions.Timeout("Custom API timeout")
        
        generator = AIInsightsGenerator(main_config_with_ai)
        insights = generator.generate_insights(sample_evaluation_result, "custom-timeout-test")
        
        assert insights == ""
        mock_post.assert_called_once()
    
    @patch('ai_eval_tool.reporting.insights.requests.post')
    def test_custom_api_http_error(self, mock_post, main_config_with_ai, sample_evaluation_result):
        ...
        generator = AIInsightsGenerator(main_config_with_ai)
        insights = generator.generate_insights(sample_evaluation_result, "custom-http-error-test")
        
        assert insights == ""
    
    @patch('ai_eval_tool.reporting.insights.requests.post')
    def test_custom_api_invalid_json_response(self, mock_post, main_config_with_ai, sample_evaluation_result):
        ...
        generator = AIInsightsGenerator(main_config_with_ai)
        insights = generator.generate_insights(sample_evaluation_result, "custom-invalid-json-test")
        
        assert insights == ""
    
    @patch('ai_eval_tool.reporting.insights.requests.post')
    def test_custom_api_missing_response_fields(self, mock_post, main_config_with_ai, sample_evaluation_result):
        ...
        mock_response = Mock()
        mock_response.raise_for_status.return_value = None
        mock_response.json.return_value = {"unexpected_field": "value"}  # Missing 'choices'
        mock_post.return_value = mock_response
        
        generator = AIInsightsGenerator(main_config_with_ai)
        insights = generator.generate_insights(sample_evaluation_result, "custom-missing-fields-test")
...
class TestNetworkConnectivityIssues:
    ...
    @patch('ai_eval_tool.reporting.insights.openai')
    def test_dns_resolution_failure(self, mock_openai, main_config_with_ai, sample_evaluation_result):
        """Test DNS resolution failure."""
        # Mock DNS resolution error
        mock_openai.ChatCompletion.create.side_effect = requests.exceptions.ConnectionError(
            "Failed to resolve hostname"
        )
        
        generator = AIInsightsGenerator(main_config_with_ai)
        insights = generator.generate_insights(sample_evaluation_result, "dns-failure-test")
        
        assert insights == ""
    
    @patch('ai_eval_tool.reporting.insights.requests.post')
    def test_ssl_certificate_error(self, mock_post, main_config_with_ai, sample_evaluation_result):
        """Test SSL certificate verification error."""
        # Configure for custom API with HTTPS
        main_config_with_ai.ai_insights.provider = "custom"
        main_config_with_ai.ai_insights.api_endpoint = "https://invalid-cert.example.com/chat"
        
        # Mock SSL error
        mock_post.side_effect = requests.exceptions.SSLError("SSL certificate verification failed")
        
        generator = AIInsightsGenerator(main_config_with_ai)
        insights = generator.generate_insights(sample_evaluation_result, "ssl-error-test")
        
        assert insights == ""
    
    @patch('ai_eval_tool.reporting.insights.openai')
    def test_network_unreachable(self, mock_openai, main_config_with_ai, sample_evaluation_result):
        ...
        generator = AIInsightsGenerator(main_config_with_ai)
        insights = generator.generate_insights(sample_evaluation_result, "network-unreachable-test")
...
class TestAPIRetryMechanisms:
    ...
    @patch('ai_eval_tool.reporting.insights.openai')
    def test_retry_on_temporary_failure(self, mock_openai, main_config_with_ai, sample_evaluation_result):
    ...
    @patch('ai_eval_tool.reporting.insights.openai')
    def test_max_retries_exceeded(self, mock_openai, mock_sleep, main_config_with_ai, sample_evaluation_result):
        """Test behavior when max retries are exceeded."""
        # Mock consistent failures
        mock_openai.ChatCompletion.create.side_effect = requests.exceptions.ConnectionError("Persistent failure")
        
        generator = AIInsightsGenerator(main_config_with_ai)
        insights = generator.generate_insights(sample_evaluation_result, "max-retries-test")
...
class TestLargePayloadHandling:
    ...
    @patch('ai_eval_tool.reporting.insights.openai')
    def test_payload_size_limit_exceeded(self, mock_openai, main_config_with_ai, sample_evaluation_result):
        """Test behavior when payload size exceeds API limits."""
        # Mock payload too large error
        mock_openai.ChatCompletion.create.side_effect = Exception("Request payload too large")
        
        generator = AIInsightsGenerator(main_config_with_ai)
        insights = generator.generate_insights(sample_evaluation_result, "payload-limit-test")
...
class TestConfigurationEdgeCases:
    ...
    def test_extreme_timeout_values(self, main_config_with_ai, sample_evaluation_result):
        ...
        with patch('ai_eval_tool.reporting.insights.openai') as mock_openai:
            mock_openai.ChatCompletion.create.side_effect = requests.exceptions.Timeout("Timeout")
            
            generator = AIInsightsGenerator(main_config_with_ai)
            insights = generator.generate_insights(sample_evaluation_result, "short-timeout-test")
        ...
        with patch('ai_eval_tool.reporting.insights.openai') as mock_openai:
            ...
            generator = AIInsightsGenerator(main_config_with_ai)
            insights = generator.generate_insights(sample_evaluation_result, "long-timeout-test")
...
class TestConcurrentAPIRequests:
    ...
    @patch('ai_eval_tool.reporting.insights.openai')
    def test_concurrent_requests_simulation(self, mock_openai, main_config_with_ai, sample_evaluation_result):
        ...
        def make_request(run_id):
            try:
                insights = generator.generate_insights(sample_evaluation_result, f"concurrent-{run_id}")
...
def test_various_network_exceptions(network_error, main_config_with_ai, sample_evaluation_result):
    """Test handling of various network-related exceptions."""
    with patch('ai_eval_tool.reporting.insights.openai') as mock_openai:
        mock_openai.ChatCompletion.create.side_effect = network_error
        
        generator = AIInsightsGenerator(main_config_with_ai)
        insights = generator.generate_insights(sample_evaluation_result, "network-error-test")
...
def test_network_recovery_after_failure(main_config_with_ai, sample_evaluation_result):
    ...
    with patch('ai_eval_tool.reporting.insights.openai') as mock_openai:
        ...
        insights1 = generator.generate_insights(sample_evaluation_result, "recovery-test-1")
        assert insights1 == ""
        
        # Second call should succeed (if retry mechanism exists)
        insights2 = generator.generate_insights(sample_evaluation_result, "recovery-test-2")
