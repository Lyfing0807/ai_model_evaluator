...
def sample_input_df() -> pl.DataFrame:
    ...
    return pl.DataFrame({
        ...
        "category_id": [0,1,0,0], "score": [0.9,0.8,0.85,0.9],
        "internal_bbox": [[[1,1,1,1]]]*4, # Placeholder list of lists
        "top_k_labels": [[["l1"]]]*4, "top_k_scores": [[[0.9]]]*4 # Placeholder
    })

def test_engine_run_detection_model(engine_detection: EvaluationEngine, sample_input_df: pl.DataFrame):
    ...
    assert isinstance(results.extra_data["deduplicated_perf_df_for_charts"], pl.DataFrame)

def test_engine_run_classification_model(engine_classification: EvaluationEngine, sample_input_df: pl.DataFrame):
    results = engine_classification.run(sample_input_df)

    assert isinstance(results, EvaluationResult)
    assert "perf_mean_total_time_ms" in results.metrics
    assert "classification_stability" in results.metrics # From DummyClassificationEvaluator

def test_engine_unsupported_model_type(test_data_dir: Path, sample_input_df: pl.DataFrame):
    config_content = {
        "project_info": {"project_name": "Unsupported Test", "model_type": "super_pose_estimator"},
    ...
    with open(config_file, 'w') as f: yaml.dump(config_content, f)
...
def test_engine_serialization_deserialization(engine_detection: EvaluationEngine, sample_input_df: pl.DataFrame, temp_cache_dir: Path):
    original_results = engine_detection.run(sample_input_df)

    # Add a DataFrame to extra_data for thorough testing of Parquet part
    original_results.extra_data["test_df_for_cache"] = pl.DataFrame({"colA": [10, 20], "colB": ["x", "y"]})
    ...
    assert (run_cache_path / "extra_data_deduplicated_perf_df_for_charts.parquet").exists()
    ...
    deserialized_df_perf = deserialized_results.extra_data["deduplicated_perf_df_for_charts"]
...
def test_engine_deserialize_non_existent_cache(engine_detection: EvaluationEngine, temp_cache_dir: Path):
    with pytest.raises(FileNotFoundError):
        engine_detection.deserialize_results(temp_cache_dir, "non_existent_run_id")

def test_engine_deserialize_corrupted_json(engine_detection: EvaluationEngine, temp_cache_dir: Path):
