...
from ai_eval_tool.utils.logging_config import setup_logging, get_logger
from ai_eval_tool.utils.types import EvaluationResult


logger = get_logger(__name__)

def execute_analysis_phase(config: MainConfig, csv_path: str, run_id: str, cache_base_path: Path, engine: EvaluationEngine):
    ...
    logger.info(f"Evaluation engine finished. Metrics: {list(evaluation_results.metrics.keys())}")

    engine.serialize_results(evaluation_results, cache_base_path, run_id)
    logger.info(f"Analysis results for Run ID {run_id} serialized to {cache_base_path / run_id}")

def execute_report_phase(config: MainConfig, run_id: str, cache_base_path: Path, engine: EvaluationEngine):
    ...
    try:
        ...
        logger.info(f"Successfully deserialized results for Run ID {run_id}. Metrics: {list(evaluation_results.metrics.keys())}")
    except FileNotFoundError:
        logger.error(f"Cache for Run ID {run_id} not found at {cache_base_path / run_id}. Cannot generate report.")
        logger.error("Please run the analysis phase first (e.g. without --report-only, or with --analyze-only).")
        sys.exit(1)
    except Exception as e:
        logger.error(f"Failed to deserialize results for Run ID {run_id}: {e}", exc_info=True)
        sys.exit(1)

    report_generator = ReportGenerator(config)
    report_generator.generate(evaluation_results, run_id)
    logger.info(f"Report generation finished for Run ID {run_id}. Reports in: {config.report_settings.output_dir.resolve()}")
...
def main_flow(args: argparse.Namespace):
    ...
    logger.info(f"Project: {config.project_info.project_name}, Model Type: {config.project_info.model_type}")
    ...
    if args.report_only and args.cache_path:
        ...
        logger.info(f"Generated Run ID: {run_id} (as it was not in config or inferred for report-only).")
    ...
    setup_logging(level=args.log_level.upper(), log_dir=Path(config.report_settings.output_dir) / run_id / "logs", run_id=run_id, console=True, file=True)
    logger.info(f"Logger re-initialized. Run ID: {run_id}. Log files in: {Path(config.report_settings.output_dir) / run_id / 'logs'}")
    ...
    if args.analyze_only:
        ...
        if not args.cache_path: # cache_path should point to the specific run_id directory
            logger.error("--cache-path (pointing to the run_id cache directory) is required for --report-only mode.")
            sys.exit(1)
        ...
        if run_specific_cache_path.name != run_id:
            logger.warning(f"Run ID inferred from cache path ({run_id}) does not match its directory name ({run_specific_cache_path.name}). Using inferred: {run_id}")
        ...
        if not args.csv:
            ...
            sys.exit(1)
        execute_analysis_phase(config, args.csv, run_id, cache_base_directory, engine)
        execute_report_phase(config, run_id, cache_base_directory, engine) # Report from the analysis just done
...
def main():
    ...
    parser.add_argument("--csv", type=str, help="Path to the input CSV data file. Required unless --report-only.")
    parser.add_argument("--log-level", type=str, default="INFO", choices=["DEBUG", "INFO", "WARNING", "ERROR", "CRITICAL"], help="Set the logging level.")

    group = parser.add_mutually_exclusive_group()
    group.add_argument("--analyze-only", action="store_true", help="Run only the analysis phase and save intermediate results. Output cache will be in <output_dir_from_config>/.cache/<run_id>/.")
    group.add_argument("--report-only", action="store_true", help="Generate report from cached intermediate results. Requires --cache-path.")

    parser.add_argument("--cache-path", type=str, help="Path to the specific run_id cache directory (e.g., <output_dir>/.cache/<your_run_id>). Required for --report-only. If provided with --analyze-only, this specific path is not directly used, but implies a cache base inside the report's output_dir.")

    args = parser.parse_args()

    if args.report_only and not args.cache_path:
        parser.error("--cache-path is required when using --report-only.")
    if (args.analyze_only or not args.report_only) and not args.csv: # Full run also needs CSV
    ...
    logger.info(f"Starting AI Model Evaluator. Config: {args.config}, CSV: {args.csv or 'N/A'}")
